...
else if(jEvent != null && jEvent.equalsIgnoreCase("Stu_Info")){
				...
				<div class="col-md-12">
					...
					%>
						...
						<td>
							...
							<a  href="LoginServlet?jEventName=G_BatchDetail&subject=Hadoop&id=<%=id %>&lp_id=<%=lpId %>&cookie_id=<%=cookie_id %>&jevent=Stu_Info" id="view_batch"><input type="button" name="b2" class="btn btn-circle blue-madison" value="View Batch Detail"  /></a>
								&nbsp;&nbsp;&nbsp;&nbsp;
...
<script src="scripts/viewStudent_editable.js"></script>
...
var sendSelfEmail=function ()
{
    	...
    	var country=ccc;
    	var name='<%=name %>';
    	window.open("Self_Paced_Email.jsp?name="+name+"&id="+id+"&email="+email+"&country="+country+"&Query_type=Login","_blank");
};
...
var sendInstructorEmail=function ()
{
      	...
      	window.open("InstructorLed_Email.jsp?name="+name+"&id="+id+"&email="+email+"&country="+country+"&interest="+interest_level+"&Query_type=Login","_blank");
};
...
$("#view").change(function(){
	...
	var link="LoginServlet?jEventName=G_BatchDetail&subject="+course+"&id=<%=id %>&lp_id=<%=lpId %>&cookie_id=<%=cookie_id %>&jevent=Stu_Info";
