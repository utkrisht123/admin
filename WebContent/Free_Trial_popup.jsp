...
String country="",pickup="",course="",date="",pagename="",country_code="";
...
//System.out.println("jEvent: "+jEvent);
else if(jEvent != null && jEvent.equalsIgnoreCase("F_Info")){
	...
	country_code=(String)request.getAttribute("country_code");
	student_id=(String)request.getAttribute("student_id");
}
			...
			<div class="row">
				<div class="col-md-12">
					...
					<tr>
						...
						<td style="width:45%">7 Days Free Trial</td>
					</tr>
					<tr>
						<td>
							 Email
					...
					<td>
						...
						<td>
							<input type="hidden" name="response_id" id="response_id" value="<%=student_id %>" />
							<a href="#" id="country" data-type="select2" data-pk="1" data-value="<%=country_code %>" data-original-title="Select country">
							</a>
						</td>
						<td>
					...
					%>
						...
						<td>
							...
							<a  href="LoginServlet?jEventName=G_BatchDetail&subject=Hadoop&id=<%=id %>&lp_id=<%=lpId %>&cookie_id=<%=cookie_id %>&jevent=free" id="view_batch"><input type="button" name="b2" class="btn btn-circle blue-madison" value="View Batch Detail"  /></a>
								&nbsp;&nbsp;&nbsp;&nbsp;
...
<script src="scripts/free_editable.js"></script>
...
var sendSelfEmail=function ()
{
	...
	var country=ccc;
	var name='<%=name %>';
	window.open("Self_Paced_Email.jsp?name="+name+"&id="+id+"&email="+email+"&country="+country+"&Query_type=7 Days Free Trial","_blank");
};
var sendInstructorEmail=function ()
{
	...
	var country=ccc;
	var name='<%=name %>';
	window.open("InstructorLed_Email.jsp?name="+name+"&id="+id+"&email="+email+"&country="+country+"&Query_type=7 Days Free Trial","_blank");
};
...
$("#view").change(function(){
	...
	var link="LoginServlet?jEventName=G_BatchDetail&subject="+course+"&id=<%=id %>&lp_id=<%=lpId %>&cookie_id=<%=cookie_id %>&jevent=free";
