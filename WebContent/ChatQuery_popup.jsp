...
String country="",pickup="",course="",date="",pagename="",Type="";
...
else if(jEvent != null && jEvent.equalsIgnoreCase("Chat_Info")){
				...
				<div class="col-md-12">
					...
					<td>
						...
						<td>
							...
							<a  href="LoginServlet?jEventName=G_BatchDetail&subject=Hadoop&id=<%=id %>&lp_id=<%=lpId %>&cookie_id=<%=cookie_id %>&jevent=Chat_Info" id="view_batch"><input type="button" name="b2" class="btn btn-circle blue-madison" value="View Batch Detail"  /></a>
								&nbsp;&nbsp;&nbsp;&nbsp;
...
<script src="scripts/viewchatQuery_editable.js"></script>
...
var sendSelfEmail=function ()
{
    	...
    	var name='<%=name %>';
    	var type='<%=Type %>';
    	window.open("Self_Paced_Email.jsp?name="+name+"&id="+id+"&email="+email+"&country="+country+"&Query_type="+type,"_blank");
};
...
var sendInstructorEmail=function ()
{
      	...
      	var type='<%=Type %>';
      	window.open("InstructorLed_Email.jsp?name="+name+"&id="+id+"&email="+email+"&country="+country+"&interest="+interest_level+"&Query_type="+type,"_blank");
};
<%--
...
$("#view").change(function(){
	...
	var link="LoginServlet?jEventName=G_BatchDetail&subject="+course+"&id=<%=id %>&lp_id=<%=lpId %>&cookie_id=<%=cookie_id %>&jevent=Chat_Info";
