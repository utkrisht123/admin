...
<div class="page-container">
	...
	<div class="page-content-wrapper">
		<div class="page-content">
			...
			<!-- BEGIN PAGE HEADER-->
			<h3 class="page-title">
			View <small> Video Details </small>
			</h3>
			<div class="page-bar">
				<ul class="page-breadcrumb">
					...
					<li>
						<a href="Vedio_Details.jsp">View Video Details</a>
					</li>
				</ul>
				...
				<div class="col-md-12">
					...
					<div class="portlet box purple">
						<div class="portlet-title">
							<div class="caption">
								<i class="fa fa-cogs"></i>View Video Details
							</div>
									...
									<div class="col-md-12">
										<form action="" method="get"  name="l_demo" id="l_demo" class="form-horizontal form-bordered">
											...
											<div id="sample_2_filter" class="dataTables_filter pull-right">
												<label>
													Search:
													<input class="form-control input-small input-inline" type="search" aria-controls="sample_2" id="nameemail" data-ng-model="searchText">
												</label>
											</div>
										</form>
									</div>
								...
								<table class="table table-striped table-bordered table-hover" id="sample_3">
									<thead>
										<tr>
											...
											<th scope="col">ENTERED_DATE</th>								                       
										</tr>
									</thead>
									<tbody  id=records_table>                       
										<tr class="odd gradeX" id=records_table data-ng-repeat="x in video|filter:searchText">	
											<td>{{ $index + 1 }}</td>							
											<td><a class='fancybox fancybox.iframe' style="color: purple;" href="LoginServlet?jEventName=V_Info&ID={{x.ID}}">{{x.NAME}}</a></td>
											<td>{{ x.EMAIL }}</td>	
											<td>{{ x.IP_ADDRESS }}</td>
											<td>{{ x.PHONE }}</td>
											<td>{{ x.PAGE_NUMBER }}</td>
											<td>{{ x.ENTERED_DATE }}</td>						
			                            </tr>                 
		                            </tbody>       
								</table>
							</div>
...
<div id="loading" class="text-center kk"   style="position:absolute;top:50%">
...
<script>
$('#date').on('DOMSubtreeModified',function(e){
		      
	 		
	        var date  =$(this).text();	       
	        var from= date.split("-");
	        var fro=$.datepicker.formatDate('yy-mm-dd', new Date(from[0]));
    		var too=$.datepicker.formatDate('yy-mm-dd', new Date(from[1]));
    		
	        angular.element($("#records_table")).scope().anyFunc(fro, too);
...
app.controller('myCtrl', function($scope,$http,$q) {
	...
	var url1="Page_Details?jEventName=video";
	
	 $http.get(url1).success(function(response){
	   $scope.video = response.records;
	 ...
	 $scope.myVar = true;
		$scope.anyFunc = function (var1, var2) {
			$("#demobatch_loading").show();
			$scope.myVar = true;
			var url2="Page_Details?jEventName=videoByFilter&d="+var1+"&d2="+var2;
			
			var promise1 = $http.get(url2).success(function(response){
				$("#demobatch_loading").hide();
				   $scope.video = response.records;
